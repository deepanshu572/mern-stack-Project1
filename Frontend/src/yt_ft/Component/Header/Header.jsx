...
const Header = () => {
  ...
  const { Loading, MobileMenu,MobileToggle ,SetMobileMenu , ToggleData , toggle } = useContext(Context);
  const nav = useNavigate();

  const svg = (
  <svg
    className={`w-[4.8rem] sm:w-[5.3rem] ${toggle ? "fill-black" : "fill-white"}`}
  ...
  return (
    <>
      <div className={`top_nav z-[999] flex items-center justify-between ${toggle ? "bg-black text-[#fff]" : "text-black bg-[#fff]" } gap-4 sm:justify-between px-[8px] py-[10px] sm:px-5 sm:py-[1rem] fixed w-full `}>
        {Loading && <Loader />}
        <div className="top_nav_left sm:mr-[10vw] flex lg:gap-5">
          {pageName !== "video" && (
            ...
            >
              {MobileMenu ? <CgClose className="w-[13px]" /> : <SlMenu className="w-[13px]" />}
            </div>
          )}

          <Link to="/">{svg}</Link>
        </div>
        <div className="top_nav_middle w-[54%] flex items-center gap-3 sm:w-1/3">
          <div className={`serch_wrap flex items-center gap-3 sm:w-full border-[2px]  ${toggle ? "bg-black " : " bg-[#fff]" } rounded-full overflow-hidden`}>
            <input
              className={` ${toggle ? "bg-black text-[#fff]" : "text-black bg-[#fff]" } text-[13px]  sm:block  border-0 outline-none px-[17px] py-[4px] sm:w-full `} 
              type="text"
              placeholder="Search"
            ...
            <div className="serch p-[7px] pl-0 pr-[12px]  ">
              <IoSearchOutline  className={`text-[20px] ${toggle ? " text-[#fff]" : "text-black " } `} />
            </div>
            {/* <SearchBar key={SearchRelatedData?.results?.value} data={SearchRelatedData} /> */}
          </div>

          {/* <div className={` ${toggle ? "bg-black text-[#fff]" : "text-black bg-[#fff]" } mic w-[35px] h-[35px] hidden   lg:flex items-center justify-center`}>
            <FaMicrophone />
          </div> */}
        </div>
        <div className="top_nav_right  items-center justify-center gap-6 sm:flex">
          <div className="video hidden">
            <RiVideoAddFill className="text-[23px]" />
          </div>
        <div className="hidden sm:inline"> <Toggle /></div>
          <div className={`account flex items-center justify-center text-[23px] ${toggle ? "bg-black text-[#fff]" : "text-black bg-[#fff]" } w-[35px] h-[35px] rounded-full `}>
